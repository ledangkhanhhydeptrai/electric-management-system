...
export default function InventorySystem() {
  ...
  const filteredData = data.filter((item) => {
    const matchesSearch =
      ...
      (item.vehicleModelName?.toUpperCase() ?? "").includes(
    ...
    // const status = getStockStatus(item); // bạn cũng cần cập nhật getStockStatus để dùng qtyOnHand và available
        ...
        <div className="space-y-6">
          {filteredData.map((item) => (
            <InventoryCard
              ...
              onView={() => router.push(`/dealer-staff/inventories/${item.id}`)}
              onEdit={() => {
