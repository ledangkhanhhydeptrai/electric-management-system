...
const Customer: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!gmailRegex.test(email)) {
      setNotification({
        open: true,
        message: "Vui lòng nhập email hợp lệ @gmail.com",
        severity: "error"
      });
      return;
    }
    ...
    if (!phoneRegex.test(phone)) {
      setNotification({
        open: true,
        message:
          "Số điện thoại không hợp lệ hoặc không được bắt đầu bằng số 0 đơn lẻ",
        severity: "error"
      });
      return;
    }
    ...
    if (dob) {
      ...
      const now = new Date();
      if (dobDate.getFullYear() > now.getFullYear()) {
        setNotification({
          open: true,
          message: "Ngày sinh không được ở năm tương lai",
          severity: "error"
        });
        return;
      }
  ...
  return (
    <>
      ...
      <div className="mt-20 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {customers.map((customer, index) => (
              <CustomerCard
                ...
                customer={customer}
                index={index}
                onView={(id) => router.push(`/dealer-staff/customer/${id}`)}
