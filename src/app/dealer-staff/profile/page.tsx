...
function translateRoleFromApi(role: string | string[]): string {
  const translateSingle = (r: string): string => {
    switch (r) {
      case "EVM_STAFF":
      case "EVM Staff":
  ...
  if (Array.isArray(role)) {
    return role.map(translateSingle).join(", ");
  }

  return translateSingle(role);
}
function translateFullNameRole(fullName: Role | Role[]): string {
  const translateSingle = (r: string): string => {
    switch (r) {
      case "EVM_STAFF":
      case "EVM Staff":
  ...
  if (Array.isArray(fullName)) {
    return fullName.map(translateSingle).join(", ");
  }

  return translateSingle(fullName);
}
export default function StaffDealerProfile() {
  useAuthGuard(["Staff"]);
  ...
  console.log("API fullName:", data.fullName);
  return (
              ...
              <div className="flex-1 text-center lg:text-left text-white space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-center lg:justify-start gap-3 mb-3">
                    ...
                    <h1 className="text-5xl font-black drop-shadow-2xl tracking-tight">
                      {translateFullNameRole(data.fullName as Role)}
                    </h1>
                    <Sparkles
                ...
                <div className="flex justify-center lg:justify-start">
                  <span className="inline-flex items-center gap-3 px-8 py-4 bg-white/20 backdrop-blur-md rounded-2xl text-lg font-bold border-2 border-white/40 shadow-2xl hover:bg-white/30 transition-all duration-300 group">
                    ...
                    {translateRoleFromApi(data.role)}
