...
import { getAllStaffForManager } from "@/services/staffServiceForManager/staffManager";
...
export default function StaffManagement() {
  ...
  const fetchStaffs = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await getAllStaffForManager();

      if (Array.isArray(response)) {
        setStaffs(response);
        setFilteredStaffs(response); // <- thêm dòng này
      } else {
        setStaffs([]);
      }
    } catch (error) {
