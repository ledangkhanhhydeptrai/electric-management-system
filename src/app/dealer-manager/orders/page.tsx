...
const OrderPage: React.FC = () => {
  ...
  const handleView = (id: string) =>
    router.push(`/dealer-manager/orders/${id}`);
  ...
  const handleSubmitOrder = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setNotification({
        open: true,
        message: errors.message || "Có lỗi xảy ra khi tạo đơn hàng",
        severity: "error"
      });
    }
  };
    ...
    <div className="mt-20 bg-gray-50">
      ...
      {notification.open &&
        <div className="fixed top-6 right-6 z-50 animate-slide-in-right">
          <div
            className={`bg-white rounded-xl shadow-2xl border-2 p-4 min-w-80 max-w-md ${notification.severity ===
            "success"
              ? "border-emerald-200"
              : "border-red-200"}`}
          >
            <div className="flex items-start gap-3">
              <div
                className={`flex items-center justify-center w-10 h-10 rounded-lg flex-shrink-0 ${notification.severity ===
                "success"
              ...
              <div className="flex-1">
                <div
                  className={`font-bold mb-1 ${notification.severity ===
                  "success"
                    ? "text-emerald-900"
                    : "text-red-900"}`}
                >
